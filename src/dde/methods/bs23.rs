...
impl<const L: usize, T: Real, V: State<T>, H: Fn(T) -> V, D: CallBackData> NumericalMethod<L, T, V, H, D> for BS23<L, T, V, H, D> {
        ...
        F: DDE<L, T, V, D>,
    {
        ...
        if L > 0 {
            ...
            for i in 0..L {
                ...
                if (t_delayed - self.t0) * self.posneg > T::default_epsilon() { // Allow for t_delayed == t0
        ...
        if self.h0 == T::zero() {
            let (h_est, h_init_evals) = h_init(
                dde, self.t, self.tf, &self.y, 3, self.rtol, self.atol, self.h_min, self.h_max, self.phi.as_ref().unwrap(), &self.k[0]
            );
            self.h0 = h_est;
            evals += h_init_evals;
        }
        ...
        F: DDE<L, T, V, D>,
    {
        ...
        for iter_idx in 0..max_iter {
            ...
            if L > 0 { dde.lags(t_new_val, &y_new_from_iter, &mut self.lags); self.lagvals(t_new_val, &y_new_from_iter); }
            ...
            if max_iter > 1 && iter_idx > 0 {
                ...
                for i_dim in 0..n_dim {
                    let scale = self.atol + self.rtol * last_y_for_errit_calc.get(i_dim).abs().max(y_new_from_iter.get(i_dim).abs());
                    if scale > T::zero() {
                        let diff_val = y_new_from_iter.get(i_dim) - last_y_for_errit_calc.get(i_dim);
        ...
        if err_final <= T::one() {
            ...
            self.cont[2] = y_diff_cont * T::from_f64(3.0).unwrap() - (k_old_for_cont * T::from_f64(2.0).unwrap() + k_new_for_cont) * self.h;
            self.cont[3] = y_diff_cont * T::from_f64(-2.0).unwrap() + (k_old_for_cont + k_new_for_cont) * self.h;
...
impl<const L: usize, T: Real, V: State<T>, H: Fn(T) -> V, D: CallBackData> BS23<L, T, V, H, D> {
    ...
    fn lagvals(&mut self, t_stage: T, _y_stage: &V) { // y_stage not used if interpolating from buffer
        for i in 0..L {
            let t_delayed = t_stage - self.lags[i];
            if (t_delayed - self.t0) * self.posneg <= T::default_epsilon() { // t_delayed <= t0 (forward) or t_delayed >= t0 (backward)
                self.yd[i] = (self.phi.as_ref().unwrap())(t_delayed);
            } else {
                ...
                for (buf_t_start, buf_t_end, buf_h, buf_cont) in self.cont_buffer.iter().rev() {
                    ...
                    if (t_delayed - *buf_t_start) * self.posneg >= -T::default_epsilon() && (t_delayed - *buf_t_end) * self.posneg <= T::default_epsilon() {
                        ...
                        self.yd[i] = buf_cont[0] + (buf_cont[1] + (buf_cont[2] + buf_cont[3] * s) * s) * s;
                ...
                if !found_in_buffer {
                    ...
                    if let Some((buf_t_start, _buf_t_end, buf_h, buf_cont)) = self.cont_buffer.back() {
                         let s = if *buf_h == T::zero() { T::one() } else { (t_delayed - *buf_t_start) / *buf_h }; // Extrapolation
                         self.yd[i] = buf_cont[0] + (buf_cont[1] + (buf_cont[2] + buf_cont[3] * s) * s) * s;
                    } else {
...
const BS23_A: [[f64; 4]; 3] = [ // A_ij * h * k_j
...
impl<const L: usize, T: Real, V: State<T>, H: Fn(T) -> V, D: CallBackData> Default for BS23<L, T, V, H, D> {
    fn default() -> Self {
        ...
        BS23 {
            ...
            a: a_conv, b: b_sol_conv, c: c_conv, er: er_conv,
            k: [V::zeros(); 4],
