...
impl<T, V, D, F> DAEProblem<T, V, D, F>
where
    T: Real,
    V: State<T>,
    D: CallBackData,
    F: DAE<T, V, D>,
{
    ...
    ///
    pub fn new(dae: F, t0: T, tf: T, y0: V) -> Self {
        DAEProblem {
            dae,
            t0,
            tf,
    ...
    pub fn solout<'a, O: Solout<T, V, D>>(
        &'a self,
        solout: &'a mut O,
    ) -> DAEProblemMutRefSoloutPair<'a, T, V, D, F, O> {
    ...
    ///
    pub fn even(&self, dt: T) -> DAEProblemSoloutPair<'_, T, V, D, F, EvenSolout<T>> {
    ...
    ///
    pub fn dense(&self, n: usize) -> DAEProblemSoloutPair<'_, T, V, D, F, DenseSolout> {
    ...
    ///
    pub fn t_eval(&self, points: Vec<T>) -> DAEProblemSoloutPair<'_, T, V, D, F, TEvalSolout<T>> {
    ...
    ) -> DAEProblemSoloutPair<'_, T, V, D, F, CrossingSolout<T>> {
        let crossing_solout =
    ...
    pub fn hyperplane_crossing<V1>(
        ...
        extractor: fn(&V) -> V1,
        direction: CrossingDirection,
    ) -> DAEProblemSoloutPair<'_, T, V, D, F, HyperplaneCrossingSolout<T, V1, V>>
    where
        V1: State<T>,
    {
        let solout =
...
pub struct DAEProblemMutRefSoloutPair<'a, T, V, D, F, O>
where
    T: Real,
    V: State<T>,
    D: CallBackData,
    ...
    O: Solout<T, V, D>,
{
    pub problem: &'a DAEProblem<T, V, D, F>, // Reference to the DAEProblem struct
    pub solout: &'a mut O,                   // Reference to the solout implementation
}

impl<'a, T, V, D, F, O> DAEProblemMutRefSoloutPair<'a, T, V, D, F, O>
where
    T: Real,
    V: State<T>,
    D: CallBackData,
    ...
    O: Solout<T, V, D>,
{
    ...
    ///
    pub fn new(problem: &'a DAEProblem<T, V, D, F>, solout: &'a mut O) -> Self {
...
#[derive(Clone, Debug)]
pub struct DAEProblemSoloutPair<'a, T, V, D, F, O>
where
    T: Real,
    V: State<T>,
    D: CallBackData,
...
impl<'a, T, V, D, F, O> DAEProblemSoloutPair<'a, T, V, D, F, O>
where
    T: Real,
    V: State<T>,
    D: CallBackData,
    ...
    O: Solout<T, V, D>,
{
    ...
    ///
    pub fn new(problem: &'a DAEProblem<T, V, D, F>, solout: O) -> Self {
