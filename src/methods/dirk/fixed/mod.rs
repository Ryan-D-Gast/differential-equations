...
macro_rules! impl_dirk_fixed_step_constructor {
    ($method_name:ident, $order_val:expr, $s_val:expr, $m_val:expr, $doc:expr) => {
        impl<E, T: Real, V: State<T>, D: CallBackData> DiagonallyImplicitRungeKutta<E, Fixed, T, V, D, $order_val, $s_val, $m_val> {
            #[doc = $doc]
...
impl_dirk_fixed_step_constructor!(esdirk33, 3, 3, 3, "ESDIRK-3-3: 3-stage, 3rd order ESDIRK method. Pairs with SSPRK(3,3) for IMEX.");
