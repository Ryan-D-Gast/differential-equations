...
macro_rules! impl_dirk_adaptive_step_constructor {
    ($method_name:ident, $order_val:expr, $s_val:expr, $m_val:expr, $doc:expr) => {
        impl<E, T: Real, V: State<T>, D: CallBackData>
            DiagonallyImplicitRungeKutta<E, Adaptive, T, V, D, $order_val, $s_val, $m_val>
        {
            #[doc = $doc]
...
impl_dirk_adaptive_step_constructor!(
    ...
    2,
    2,
    "SDIRK-2-1: 2-stage, 2(1) order SDIRK method with embedded error estimation. L-stable."
);
impl_dirk_adaptive_step_constructor!(
    esdirk324l2sa,
    3,
    4,
    4,
    "ESDIRK3(2)4L[2]SA: 4-stage, 3(2) order ESDIRK method with embedded error estimation. High-quality L-stable method."
);
impl_dirk_adaptive_step_constructor!(
    kvaerno423,
    3,
    4,
    4,
    "Kvaerno(4,2,3): 4-stage, 3(2) order ESDIRK method with embedded error estimation. High-quality L-stable method."
);
impl_dirk_adaptive_step_constructor!(
    kvaerno745,
    5,
    7,
    7,
    "Kvaerno(7,4,5): 7-stage, 5(4) order ESDIRK method with embedded error estimation. High-order with excellent stability."
);
