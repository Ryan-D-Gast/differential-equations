    ...
    OrdinaryNumericalMethod<T, Y> for ExplicitRungeKutta<Ordinary, Adaptive, T, Y, O, S, I>
{
        ...
        F: ODE<T, Y>,
    {
        ...
        for i in 1..self.stages {
            ...
            for j in 0..i {
                y_stage += self.k[j].clone() * (self.a[i][j] * self.h);
            }
        ...
        for i in 0..self.stages {
            y_high += self.k[i].clone() * (self.b[i] * self.h);
        }
        ...
        for i in 0..self.stages {
            y_low += self.k[i].clone() * (bh[i] * self.h);
        }
            ...
            if self.bi.is_some() {
                ...
                for i in 0..(I - S) {
                    ...
                    for j in 0..self.stages + i {
                        y_stage += self.k[j].clone() * (self.a[self.stages + i][j] * self.h);
                    }
    ...
    for ExplicitRungeKutta<Ordinary, Adaptive, T, Y, O, S, I>
{
    fn interpolate(&mut self, t_interp: T) -> Result<Y, Error<T, Y>> {
        ...
        if let Some(bi) = self.bi.as_ref() {
            ...
            for i in 0..I {
                y_interp += self.k[i].clone() * cont[i] * self.h_prev;
            }
