    ...
    F: ODE<T, R, C, E>,
{
        ...
        S: Solver<T, R, C, E>,
    {
        ...
        solve_ivp(solver, &self.ode, self.t0, self.tf, &self.y0, &mut default_solout)
    }
    ...
    /// 
    pub fn crossing(&self, component_idx: usize, threshhold: T, direction: CrossingDirection) -> IVPSoloutPair<'_, T, R, C, E, F, CrossingSolout<T>> {
    ...
    O: Solout<T, R, C, E>,
{
    ...
    where 
        S: Solver<T, R, C, E>,
    {
        solve_ivp(solver, &self.ivp.ode, self.ivp.t0, self.ivp.tf, &self.ivp.y0, self.solout)
    }
}
    ...
    O: Solout<T, R, C, E>,
{
    ...
    where 
        S: Solver<T, R, C, E>,
    {
        solve_ivp(solver, &self.ivp.ode, self.ivp.t0, self.ivp.tf, &self.ivp.y0, &mut self.solout)
    }
}