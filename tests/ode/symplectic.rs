...
fn test_symplectic_odd_dimension_returns_error() {
    ...
    let result_vv = IVP::ode(&system, 0.0, 1.0, y0_odd.clone())
    ...
    let result_rf = IVP::ode(&system, 0.0, 1.0, y0_odd)
...
fn test_symplectic_empty_dimension_returns_error() {
    let system = SimpleHarmonicOscillator;
    let y0_empty = vec![];
    let result = IVP::ode(&system, 0.0, 1.0, y0_empty)
        .method(SymplecticIntegrator::velocity_verlet(0.1))
        .solve();
...
#[test]
fn test_symplectic_solves_harmonic_oscillator() {
    let system = SimpleHarmonicOscillator;
    let y0 = vec![1.0, 0.0]; // q=1, p=0

    let solution_vv = IVP::ode(&system, 0.0, 2.0 * std::f64::consts::PI, y0.clone())
        .method(SymplecticIntegrator::velocity_verlet(0.01))
        .solve()
        .expect("Velocity Verlet should solve");
    ...
    let solution_rf = IVP::ode(&system, 0.0, 2.0 * std::f64::consts::PI, y0)
        .method(SymplecticIntegrator::ruth_forest(0.01))
        .solve()
        .expect("Ruth-Forest should solve");
...
fn test_symplectic_linear_interpolation() {
    ...
    solver.init(&system, 0.0, 1.0, &y0).unwrap();
    solver.step(&system).unwrap();
